...
# Function to convert string to lowercase (macOS compatible)
to_lowercase() {
...
if [ ${#new_pdf_files[@]} -gt 1 ]; then
    ...
    for file in "${new_pdf_files[@]}"; do
        # macOS compatible stat command (BSD stat)
        if command -v gstat >/dev/null 2>&1; then
            # GNU stat is available (installed via brew)
            file_time=$(gstat -c %Y "$file" 2>/dev/null)
        else
            # Use BSD stat (default on macOS)
            file_time=$(stat -f %m "$file" 2>/dev/null)
        fi
        
        if [ -z "$file_time" ]; then
            error_exit "Failed to get modification time for $file"
        fi
